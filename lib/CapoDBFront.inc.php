...
class CapoDBFront
{
    ...
    function GetRecipes($limit = NULL)
    {
        ...
        $prep = $this->SQL->conn->query("SELECT RecipesIndex.title, RecipesCategories.name as category, RecipesIndex.created_by, RecipesIndex.created_on, RecipesIndex.hash FROM RecipesCategories, RecipesIndex WHERE RecipesCategories.id = RecipesIndex.category ORDER BY RecipesIndex.id ASC LIMIT $limit");
        #$prep->execute();
        var_dump($prep);
        $this->AllRecipes = $prep->fetchAll(2);
    }

    function GetRecipeData($hash = NULL)
    {
